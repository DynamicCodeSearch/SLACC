...
def syntactic_differences(start=0):
  ...
  stmts = syntactic.get_normalized_py_statements(debug=True)
  trees = {}
  for i in xrange(len(stmts)-1):
    if i < start:
      continue
    LOGGER.info("Processing %d / %d Python snippet ... " % (i + 1, len(stmts)))
    stmt_i = stmts[i]
    stmt_i_tree = trees.get(i, None)
    if not stmt_i_tree:
      stmt_i_tree = ast_distances.py_parse(stmt_i.normalized)
      trees[i] = stmt_i_tree
    records = []
    for j in xrange(i+1, len(stmts)):
      ...
      if not stmt_j_tree:
        stmt_j_tree = ast_distances.py_parse(stmt_j.normalized)
        trees[j] = stmt_j_tree
      record = {
        "id_1": stmt_i.mongo_id,
        "id_2": stmt_j.mongo_id,
        "language": props.TYPE_PYTHON,
