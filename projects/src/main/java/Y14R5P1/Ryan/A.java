...
public class A{
  ...
  private static long choose(long[] partials,int a,int b,int n){
  ...
  private static long choose(long[] partials,int i,int n){
  ...
  private static double solve(Scanner scanner){
    ...
    for(int i=1;i<=n;i++)
      partials[i]=partials[i-1]+array[i-1];
