package CodeJam.Y11R5P1.eatmore;
...
public class A {
    ...
    static void solveTest() throws Exception {
        ...
        double ny[] = new double[nx.length];
        {
            ...
            int iu = 0;
            while (true) {
                double nyl = nx[i] > xl[il] ? yl[il] + (double) (yl[il + 1] - yl[il]) * (nx[i] - xl[il]) / (xl[il + 1] - xl[il]) : yl[il];
                double nyu = nx[i] > xu[iu] ? yu[iu] + (double) (yu[iu + 1] - yu[iu]) * (nx[i] - xu[iu]) / (xu[iu + 1] - xu[iu]) : yu[iu];
                ny[i] = nyu - nyl;
                ++i;
                if (i >= nx.length) {
                    break;
