package CodeJam.Y11R5P1.mikhailOK;
...
public class A {
    ...
    void solve() throws IOException {
        ...
        for (int i = 0; i < events.length - 1; i++) {
            ...
            double d1 = getY(high[b], high[b + 1], x1) - getY(low[a], low[a + 1], x1);
            double d2 = getY(high[b], high[b + 1], x2) - getY(low[a], low[a + 1], x2);
            // System.out.println(d1 + " " + d2 + " asf ");
            double ok = (d1 + d2) * .5 * (x2 - x1);
            // System.out.println(ok + " ok " + slice + " " + x1 + " " + x2);
            // System.out.println(x1 + " " + x2 + " -> " + ok + " " + d1 + " " +
            // d2);
            // System.out.println(cur);
            double x_1 = x1;
            while (cur + ok >= slice && res.size() < guests - 1) {
                double xx = getX(d1, d2, x2 - x_1, slice - cur) + x_1;
                // System.out.println("CUT AT " + xx);
                res.add(xx);
                // System.out.println("SLICE");
                x_1 = xx;
        ...
        if (Math.abs(cur - slice) >= 1e-6) {
            System.out.println("BAD " + cur + " " + slice);
        }
        out.println();
        for (double d : res) {
            out.println(d);
