...
public class A {
    ...
    double trapS(double xl, double xr, double yl1, double yl2, double yr1, double yr2) {
        return (yl2 - yl1 + yr2 - yr1) / 2 * (xr - xl);
    }

    double yAtX(double x, int[] xs, int[] ys) {
        // find between
    ...
    double calc(double fromX, double toX, int[] lx, int[] ly, int[] ux, int[] uy) {
        double cx = fromX;
        double s = 0;
        while (cx < toX) {
            // find next x
