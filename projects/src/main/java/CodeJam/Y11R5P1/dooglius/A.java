package CodeJam.Y11R5P1.dooglius;
...
public class A {
    ...
    static double findPoint(Trap[] arr, double area) {
    ...
    static class Trap {
        ...
        public double part(double needed) {
            if (Math.abs(right - left) < .000001)
                return needed / area;
            else
                return (Math.sqrt(left * left + 2 * needed * (right - left)) - left) / (right - left);
        }
    }
}
