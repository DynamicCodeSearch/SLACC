package CodeJam.Y13R5P1.jpaulson;
...
public class A {
    ...
    static long money_counted(Long[] A, int nuse, long min) {
        ...
        for (int i = 0; i < nuse; i++) money_counted += min - A[i];
        return money_counted;
    }

    static long money_used(Long[] A, int nuse, long min) {
        long money_used = 0;
        for (int i = 0; i < nuse; i++) money_used += min - A[i];
        for (int i = nuse; i < 37; i++) if (A[i] <= min + 1)
