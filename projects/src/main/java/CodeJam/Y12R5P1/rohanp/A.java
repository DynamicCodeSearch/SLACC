package CodeJam.Y12R5P1.rohanp;
...
public class A {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int t = 1; t <= test; t++) {
            ...
            for (int i = 0; i < num; i++) arr[i] = new Level(readInt(), i);
            for (int i = 0; i < num; i++) arr[i].percent = readInt();
            double soFar = 0;
            boolean flag = true;
            while (flag) {
                flag = false;
            ...
            for (int i = 0; i < num; i++) out.print(" " + arr[i].index);
            out.println();
        }
        out.close();
        in.close();
