package CodeJam.Y12R5P1.immoonancient;
...
public class ProbA {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        for (int caseI = 1; caseI <= caseN; caseI++) {
            ...
            int[] l = new int[n];
            int[] p = new int[n];
            for (int i = 0; i < n; i++) l[i] = cin.nextInt();
            for (int i = 0; i < n; i++) p[i] = cin.nextInt();
            GameLevel[] levels = new GameLevel[n];
            for (int i = 0; i < n; i++) levels[i] = new GameLevel(i, l[i], p[i]);
            Arrays.sort(levels);
            cout.print("Case #" + caseI + ":");
            for (int i = 0; i < n; i++) cout.print(" " + levels[i].getId());
            cout.println();
        }
    }
}
