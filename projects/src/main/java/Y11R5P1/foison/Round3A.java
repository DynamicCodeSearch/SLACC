...
public class Round3A {
	...
	public static class Cross implements Comparable<Cross> {
		Cross(double x, double low, double high) { this.x = x; this.low = low; this.high = high; }
		public double x;
		public double low;
		public double high;
	...
	public static String handleCase() {
		...
		for (int i = 0; i < g-1; i++) {
			...
			while (chunkLeft > 0.0000000001) {
				double crossArea = (crosses.get(crossInd).x - currentX) *
						(((currentHigh - currentLow) + (crosses.get(crossInd).high - crosses.get(crossInd).low)) / 2.0);
				...
				if (chunkLeft > crossArea) {
					...
					for (int j = 0; j < 100; j++) {
						...
						double thisArea = (thisX - currentX) *
							(((currentHigh - currentLow) + (thisHigh - thisLow)) / 2.0);
