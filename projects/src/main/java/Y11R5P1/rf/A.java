...
public class A {
	...
	void solve() throws IOException {
		...
		for (int i = 0; i < down; i++)
			downB[i] = new Point(nextInt(), nextInt());
		
		Point[] upB = new Point[up];
		for (int i = 0; i < up; i++)
			upB[i] = new Point(nextInt(), nextInt());
		
		
		
		ArrayList<Point> res = new ArrayList<A.Point>();
...
//		}
		...
		for (int i = 0; i < res.size() - 1; i++)
			piece += 0.5 * (res.get(i + 1).y + res.get(i).y) * (res.get(i + 1).x - res.get(i).x);
